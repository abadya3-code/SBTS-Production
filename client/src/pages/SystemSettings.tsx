...
import { TagLayoutDesigner } from "@/components/settings/TagLayoutDesigner";
import {
  WorkflowGuardPolicies,
  type WorkflowGuardPolicySection,
} from "@/components/settings/WorkflowGuardPolicies";
import {
  parseTagLayoutJson,
  parseTagTemplateSlotsJson,
  sanitizeTagLayout,
  serializeTagLayout,
  serializeTagTemplateSlots,
  type TagLayoutV1,
...
const tabs: {
  ...
  },
  {
    ...
    icon: Workflow,
    description: "Canonical workflow, gates and plant safety policies",
  },
  {
    key: "defaultTag",
  ...
  },
  {
    ...
    icon: ToggleLeft,
    description: "Enable/disable Blind Hub features",
  },
];
...
function GeneralSettingsTab() {
  ...
  const [form, setForm] = useState<{
    ...
    timezone: string;
    dateFormat: string;
    defaultTheme: "standard" | "modern" | "manus";
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      defaultTheme:
        (data.defaultTheme as "standard" | "modern" | "manus") ?? "standard",
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="sbts-card">
        <CardHeader className="pb-4">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-violet-50 ring-1 ring-violet-100">
            ...
            <div>
              ...
              <CardDescription className="text-xs text-slate-500">
                Keep the visual system consistent across dashboards, field pages
                and certificates.
              </CardDescription>
            </div>
          </div>
              ...
              onValueChange={value =>
                setForm(
                  current =>
                    current && {
                      ...current,
                      defaultTheme: value as "standard" | "modern" | "manus",
                    }
                )
              }
            >
              ...
              <SelectContent>
                <SelectItem value="standard">
                  Standard · light command center
                </SelectItem>
                <SelectItem value="modern">Modern · dark industrial</SelectItem>
                <SelectItem value="manus">Manus · premium violet</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
            <div>
              ...
              <div className="text-xs text-slate-500">
                When disabled, the plant default is enforced for every account.
              </div>
            </div>
            <Switch
              ...
              onCheckedChange={value =>
                setForm(
                  current =>
                    current && { ...current, allowUserThemeOverride: value }
                )
              }
            />
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="Professional Edition"
                className="sbts-input"
              />
            </div>
            <div className="space-y-1.5">
...
function DefaultTagSettingsTab() {
  ...
  useEffect(() => {
    if (!data) return;
    const widthMm = (data as any).tagWidth ?? 70;
    const heightMm = (data as any).tagHeight ?? 110;
    const rawLayoutJson = (data as any).layoutJson as string | null | undefined;
    let layout = parseTagLayoutJson(rawLayoutJson, { widthMm, heightMm });
    if (!rawLayoutJson?.trim()) {
      layout = sanitizeTagLayout({
        ...layout,
        canvas: {
          ...layout.canvas,
          borderColor: (data as any).tagColor ?? "#0f172a",
        },
        hole: {
          ...layout.hole,
          enabled: ((data as any).tagHoleEnabled ?? 1) === 1,
        },
        elements: layout.elements.map(element => ({
          ...element,
          visible:
            element.kind === "qr"
              ? ((data as any).tagShowQR ?? 1) === 1
              : element.kind === "logo"
                ? ((data as any).tagShowLogo ?? 1) === 1
                : element.visible,
          color:
            element.kind === "qr" || element.kind === "logo"
              ? element.color
              : ((data as any).tagFontColor ?? "#0f172a"),
          fontSizePt:
            element.kind === "id"
              ? ((data as any).tagFontSize ?? 14)
              : element.fontSizePt,
        })),
      });
    }
    setForm({
      ...
      tagColor: layout.canvas.borderColor,
      tagWidth: layout.canvas.widthMm,
      tagHeight: layout.canvas.heightMm,
      tagFontSize: Math.round(
        layout.elements.find(element => element.kind === "id")?.fontSizePt ?? 14
      ),
      tagFontColor:
        layout.elements.find(element => element.kind === "id")?.color ??
        "#0f172a",
      tagTheme: (data as any).tagTheme ?? "industrial",
      tagShowLogo:
        layout.elements.find(element => element.kind === "logo")?.visible ??
        true,
      tagShowQR:
        layout.elements.find(element => element.kind === "qr")?.visible ?? true,
      tagHoleEnabled: layout.hole.enabled,
      tagHolePosition: "top-center",
      layout,
      templateSlots: parseTagTemplateSlotsJson((data as any).templateSlotsJson),
    });
  }, [data]);
  ...
  const handleSave = () => {
    updateMutation.mutate({
      ...
      layoutJson: serializeTagLayout(form.layout),
      templateSlotsJson: serializeTagTemplateSlots(form.templateSlots),
    } as any);
  };

  const handleLayoutChange = (nextLayout: TagLayoutV1) => {
    const layout = sanitizeTagLayout(nextLayout);
    const idElement = layout.elements.find(element => element.kind === "id");
    setForm(current =>
      current
        ? {
            ...current,
            ...
            tagHeight: layout.canvas.heightMm,
            tagColor: layout.canvas.borderColor,
            tagFontColor: idElement?.color ?? current.tagFontColor,
            tagFontSize: Math.round(
              idElement?.fontSizePt ?? current.tagFontSize
            ),
            tagShowQR:
              layout.elements.find(element => element.kind === "qr")?.visible ??
              false,
            tagShowLogo:
              layout.elements.find(element => element.kind === "logo")
                ?.visible ?? false,
            tagHoleEnabled: layout.hole.enabled,
            tagHolePosition: "top-center",
          }
  ...
  return (
    <div className="space-y-6">
      <TagLayoutDesigner
        layout={form.layout}
        templateSlots={form.templateSlots}
        onLayoutChange={handleLayoutChange}
        onTemplateSlotsChange={templateSlots =>
          setForm(current =>
            current ? { ...current, templateSlots } : current
          )
        }
        sampleValues={{
          id: previewTag,
          size: `Size · ${form.defaultSize || "—"}`,
          rating: `Rating · ${form.defaultRate || "—"}`,
        }}
      />
                ...
                onChange={e =>
                  setForm(
                    f =>
                      f && {
                        ...f,
                        minPasswordLength: parseInt(e.target.value) || 12,
                      }
                  )
                }
                className="sbts-input"
...
function NotificationSettingsTab() {
  ...
  const [form, setForm] = useState<{
    ...
    workflowTransition: boolean;
    workflowGateBlocked: boolean;
    workflowApprovalRequired: boolean;
    safetyHoldPlaced: boolean;
    safetyHoldReleased: boolean;
    qrTokenChanged: boolean;
    certificateStatusChanged: boolean;
    tagPrintRequested: boolean;
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      qrTokenChanged: data.qrTokenChanged === 1,
      certificateStatusChanged: data.certificateStatusChanged === 1,
      tagPrintRequested: data.tagPrintRequested === 1,
  ...
  const categories = [
    ...
    {
      title: "QR, Certificates & Printing",
      items: [
        {
          key: "qrTokenChanged" as const,
          label: "QR Token Governance",
          desc: "When secure Blind QR tokens are issued, rotated, or revoked",
        },
        {
          key: "certificateStatusChanged" as const,
          label: "Certificate Status",
          desc: "When governed certificates are issued or revoked",
        },
        {
          key: "tagPrintRequested" as const,
          label: "Controlled Tag Printing",
          desc: "When a physical Blind Tag print or PDF package is prepared",
        },
      ],
    },
    {
    ...
    {
      title: "Canonical Workflow & Safety",
      items: [
        {
          key: "workflowTransition" as const,
          label: "Workflow Transition",
          desc: "When a Blind completes a canonical phase and moves to the next owner",
        },
        {
          key: "workflowGateBlocked" as const,
          label: "Gate Blocked",
          desc: "When server validation rejects a phase action",
        },
        {
          key: "workflowApprovalRequired" as const,
          label: "Approval Required",
          desc: "When the next final-approval role must act",
        },
        {
          key: "safetyHoldPlaced" as const,
          label: "Safety Hold Placed",
          desc: "When Stop Work freezes a Blind and its Isolation Package",
        },
        {
          key: "safetyHoldReleased" as const,
          label: "Safety Hold Released",
          desc: "When corrective action is independently accepted",
        },
      ],
    },
  ];
...
function WorkflowSafetySettingsTab() {
  const { data, isLoading, refetch } =
    trpc.settings.workflowPolicy.get.useQuery();
  ...
  const updateMutation = trpc.settings.workflowPolicy.update.useMutation({
    onSuccess: () => {
      toast.success("Workflow and safety policies saved.");
      refetch();
    },
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      holdReleaseRequiresIndependentApproval:
        data.holdReleaseRequiresIndependentApproval === 1,
      metalForemanRequiredForSlipBlind:
        data.metalForemanRequiredForSlipBlind === 1,
      ...
      requireIsolationPackageForEntry:
        data.requireIsolationPackageForEntry === 1,
      ...
      requireSequentialFinalApprovals:
        data.requireSequentialFinalApprovals === 1,
      requireLotoReleasedForCloseout: data.requireLotoReleasedForCloseout === 1,
      blockTransitionWhenPermitExpired:
        data.blockTransitionWhenPermitExpired === 1,
      ...
      workflowUiDensity:
        (data.workflowUiDensity as "comfortable" | "compact") ?? "comfortable",
      safetyBannerMode:
        (data.safetyBannerMode as "prominent" | "standard" | "compact") ??
        "prominent",
      authorizedGasTesterRoleKey:
        data.authorizedGasTesterRoleKey ?? "gasTester",
      gasTestRequiresInstrumentCalibration:
        data.gasTestRequiresInstrumentCalibration === 1,
      gasTestLimitsConfigured: data.gasTestLimitsConfigured === 1,
      gasTestOxygenMinPercent:
        data.gasTestOxygenMinPercent == null
          ? null
          : Number(data.gasTestOxygenMinPercent),
      gasTestOxygenMaxPercent:
        data.gasTestOxygenMaxPercent == null
          ? null
          : Number(data.gasTestOxygenMaxPercent),
      gasTestMaxLelPercent:
        data.gasTestMaxLelPercent == null
          ? null
          : Number(data.gasTestMaxLelPercent),
      gasTestMaxH2sPpm:
        data.gasTestMaxH2sPpm == null ? null : Number(data.gasTestMaxH2sPpm),
      gasTestMaxCoPpm:
        data.gasTestMaxCoPpm == null ? null : Number(data.gasTestMaxCoPpm),
      ...
      certificateRequireClosedWorkflow:
        data.certificateRequireClosedWorkflow === 1,
      certificateReissueRequiresReason:
        data.certificateReissueRequiresReason === 1,
      ...
      requireDefectDispositionBeforeClosure:
        data.requireDefectDispositionBeforeClosure === 1,
      requireMandatoryPunchClosureBeforeReadyForClosure:
        data.requireMandatoryPunchClosureBeforeReadyForClosure === 1,
      requireNdtAcceptanceBeforeReadyForClosure:
        data.requireNdtAcceptanceBeforeReadyForClosure === 1,
  ...
  const activeWorkflow = workflows?.find(
    workflow => workflow.id === form.activeWorkflowTemplateId
  );
  ...
  const policyRows: {
    ...
    {
      key: "enforceServerGates",
      label: "Enforce server-side transition gates",
      description:
        "The API must reject progression when a mandatory requirement is incomplete.",
      critical: true,
    },
    {
      key: "requireIndependentVerifier",
      label: "Independent mechanical verification",
      description:
        "Executing technician and independent verifier must remain separate.",
      critical: true,
    },
    {
      key: "requirePtwActive",
      label: "Active PTW required",
      description:
        "Block applicable phase actions when the permit is missing or expired.",
      critical: true,
    },
    {
      key: "requireLotoActive",
      label: "Active LOTO required",
      description:
        "Require verified energy isolation during controlled installation and removal.",
      critical: true,
    },
    {
      key: "requireGasTestForEntry",
      label: "Gas test mandatory for entry readiness",
      description:
        "Prevent entry authorization when the atmospheric test is invalid.",
      critical: true,
    },
    {
      key: "requireGasTestForDeBlinding",
      label: "Gas test mandatory before de-blinding",
      description:
        "Apply the configured gas-test rule before blind removal where required.",
    },
    {
      key: "requireGasTestForLineBreaking",
      label: "Gas test mandatory for line breaking",
      description:
        "Require a valid line-breaking gas test before containment is opened.",
    },
    {
      key: "gasTestRequiresInstrumentCalibration",
      label: "Calibrated gas-test instrument required",
      description:
        "Active or valid gas tests require an instrument ID and non-expired calibration.",
      critical: true,
    },
    {
      key: "requireLineBreakingPermit",
      label: "Line Breaking Permit required",
      description:
        "Block installation and removal work when the line-breaking permit is missing or invalid.",
      critical: true,
    },
    {
      key: "requireIsolationPackageForEntry",
      label: "Isolation Package required for entry",
      description:
        "Entry authorization must be based on all linked blind points, not a single blind.",
      critical: true,
    },
    {
      key: "preventBlindInMultipleActivePackages",
      label: "One active package per blind",
      description:
        "Prevent the same blind from being linked to more than one open Isolation Package.",
      critical: true,
    },
    {
      key: "requireTorqueCalibration",
      label: "Valid torque calibration required",
      description:
        "Reject torque records when the selected tool calibration is missing or expired.",
      critical: true,
    },
    {
      key: "requireInstallationTorque",
      label: "Installation torque record required",
      description:
        "Require a complete installation torque record before mechanical verification.",
    },
    {
      key: "requireReinstatementTorque",
      label: "Reinstatement torque record required",
      description:
        "Require a second torque record after blind removal or repositioning.",
    },
    {
      key: "safetyHoldEnabled",
      label: "Safety Hold / Stop Work",
      description:
        "Allow authorized field roles to freeze progression for an unsafe condition.",
      critical: true,
    },
    {
      key: "holdReleaseRequiresIndependentApproval",
      label: "Independent Safety Hold release",
      description:
        "Corrective action and a separate approval are required before release.",
    },
    {
      key: "metalForemanRequiredForSlipBlind",
      label: "Metal Foreman approval for slip blinds",
      description:
        "Add a conditional mandatory approval for spade/slip-blind work.",
    },
    {
      key: "operationsForemanFinalApprover",
      label: "Operations Foreman is final approver",
      description:
        "Return-to-service authorization remains the last approval in the chain.",
      critical: true,
    },
    {
      key: "certificateRequiresLeakTest",
      label: "Leak test required for final certificate",
      description:
        "Prevent final certificate locking until the leak/service test passes.",
      critical: true,
    },
    {
      key: "requireSequentialFinalApprovals",
      label: "Sequential final approvals",
      description:
        "Inspection, T&I, mechanical/metal foreman and Operations must approve in order.",
    },
    {
      key: "requireLotoReleasedForCloseout",
      label: "LOTO closeout required",
      description:
        "Do not close or lock the workflow until controlled LOTO release is recorded.",
      critical: true,
    },
    {
      key: "blockTransitionWhenPermitExpired",
      label: "Block expired permits",
      description:
        "The runtime engine rejects a transition when an applicable permit has expired.",
    },
    {
      key: "allowAdminWorkflowOverride",
      label: "Allow controlled Admin override",
      description:
        "Permit documented emergency override only for administrators. Keep disabled for normal production.",
    },
    {
      key: "allowPhaseReopen",
      label: "Allow approved phase reopening",
      description:
        "Permit controlled correction without deleting the original record.",
    },
    {
      key: "phaseReopenRequiresApproval",
      label: "Approval required to reopen a phase",
      description:
        "Record reason, requester and approver before creating a new revision.",
    },
    {
      key: "showBlockingReasons",
      label: "Show blocking reasons in the UI",
      description:
        "Explain exactly which requirement prevents the current action.",
    },
    {
      key: "showGateReadinessPanel",
      label: "Show Gate Readiness panel",
      description: "Display live server validation results on Blind Detail.",
    },
    {
      key: "showLegacyPhaseReference",
      label: "Show legacy phase reference",
      description:
        "Display the five-phase compatibility value during migration. Disable after cutover.",
    },
    {
      key: "enableFieldMode",
      label: "Enable tablet/mobile field mode",
      description:
        "Use field-oriented actions, large targets and concise current-phase content.",
    },
    {
      key: "requireEvidenceBeforePhaseSubmit",
      label: "Require evidence before phase submission",
      description:
        "Block phase submission until at least one current-phase evidence record is attached.",
    },
  ];

  const policyByKey = new Map(policyRows.map(item => [item.key, item]));
  const selectPolicies = (
    ...keys: BooleanPolicyKey[]
  ): WorkflowGuardPolicySection<BooleanPolicyKey>["items"] =>
    keys.flatMap(key => {
      const item = policyByKey.get(key);
      return item ? [item] : [];
    });

  const policySections: WorkflowGuardPolicySection<BooleanPolicyKey>[] = [
    {
      id: "scope-template",
      title: "Scope & Template",
      description:
        "Control migration visibility while the selected canonical template defines project workflow scope.",
      items: selectPolicies("showLegacyPhaseReference"),
    },
    {
      id: "core-server-gates",
      title: "Core Server Gates",
      description:
        "Authoritative API enforcement and expiry checks that protect every workflow transition.",
      items: selectPolicies(
        "enforceServerGates",
        "blockTransitionWhenPermitExpired"
      ),
    },
    {
      id: "ptw-loto-line-breaking",
      title: "PTW / LOTO / Line Breaking",
      description:
        "Permit-to-work, energy isolation, and containment-breaking prerequisites.",
      items: selectPolicies(
        "requirePtwActive",
        "requireLotoActive",
        "requireLineBreakingPermit"
      ),
    },
    {
      id: "gas-testing",
      title: "Gas Testing",
      description:
        "Atmospheric testing requirements for entry, line breaking, and de-blinding work.",
      items: selectPolicies(
        "requireGasTestForEntry",
        "requireGasTestForDeBlinding",
        "requireGasTestForLineBreaking",
        "gasTestRequiresInstrumentCalibration"
      ),
    },
    {
      id: "isolation-entry",
      title: "Isolation Packages & Entry",
      description:
        "Package-level readiness and exclusive blind membership for safe vessel entry.",
      items: selectPolicies(
        "requireIsolationPackageForEntry",
        "preventBlindInMultipleActivePackages"
      ),
    },
    {
      ...
      title: "Mechanical / Torque / Evidence",
      description:
        "Independent verification, calibrated torque records, and phase evidence controls.",
      items: selectPolicies(
        "requireIndependentVerifier",
        "requireTorqueCalibration",
        "requireInstallationTorque",
        "requireReinstatementTorque",
        "requireEvidenceBeforePhaseSubmit"
      ),
    },
    {
      id: "holds-reopen-override",
      title: "Holds / Reopen / Override",
      description:
        "Controlled stop-work, corrective revision, and emergency administrative override paths.",
      items: selectPolicies(
        "safetyHoldEnabled",
        "holdReleaseRequiresIndependentApproval",
        "allowPhaseReopen",
        "phaseReopenRequiresApproval",
        "allowAdminWorkflowOverride"
      ),
    },
    {
      id: "final-closeout",
      title: "Final Approvals / Closeout",
      description:
        "Approval order, leak-test evidence, LOTO release, and return-to-service authority.",
      items: selectPolicies(
        "metalForemanRequiredForSlipBlind",
        "operationsForemanFinalApprover",
        "certificateRequiresLeakTest",
        "requireSequentialFinalApprovals",
        "requireLotoReleasedForCloseout"
      ),
    },
    {
      id: "field-ui",
      title: "Field UI",
      description:
        "Operator-facing readiness details, blocking reasons, and tablet/mobile field behavior.",
      items: selectPolicies(
        "showBlockingReasons",
        "showGateReadinessPanel",
        "enableFieldMode"
      ),
    },
  ];
  ...
  const policyValues = Object.fromEntries(
    policyRows.map(item => [item.key, Boolean(form[item.key])])
  ) as Record<BooleanPolicyKey, boolean>;
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-cyan-200 bg-gradient-to-r from-cyan-50 to-white p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.18em] text-cyan-700">
              <CheckCircle2 className="h-4 w-4" /> Sprint 2 Runtime Foundation
            </div>
            <h3 className="mt-2 text-xl font-black text-slate-950">
              Canonical 8-Phase Isolation Lifecycle
            </h3>
            <p className="mt-1 max-w-3xl text-sm text-slate-600">
              This setting selects the database workflow template enforced by
              the server state machine. Legacy phases remain only as a
              synchronized compatibility projection during controlled migration.
            </p>
          </div>
          <Badge className="w-fit rounded-full bg-emerald-100 px-3 py-1 text-emerald-800">
            Database backed
          </Badge>
        </div>
      </div>
      ...
      <Card className="sbts-card">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base font-extrabold">
            <Workflow className="h-5 w-5 text-cyan-700" /> Active Workflow
            Template
          </CardTitle>
          <CardDescription>
            Select the workflow definition assigned to projects and enforced by
            the runtime transition engine.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Select
            value={form.activeWorkflowTemplateId}
            onValueChange={value =>
              setForm(current =>
                current
                  ? { ...current, activeWorkflowTemplateId: value }
                  : current
              )
            }
          >
            ...
            <SelectContent>
              {(workflows ?? [])
                .filter(workflow => workflow.id === "wf-sbts-standard-v2")
                .map(workflow => (
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="sbts-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base font-extrabold">
              <Timer className="h-5 w-5 text-amber-600" /> Gas-Test Timing
            </CardTitle>
            <CardDescription>
              Default validity and warning windows. Site rules can be refined
              later by phase and service.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label>Default validity (minutes)</Label>
              <Input
                type="number"
                min={5}
                max={1440}
                value={form.defaultGasTestValidityMinutes}
                onChange={event =>
                  setForm(current =>
                    current
                      ? {
            ...
            <div className="space-y-1.5">
              <Label>Expiry warning (minutes)</Label>
              <Input
                type="number"
                min={1}
                max={240}
                value={form.gasTestExpiryWarningMinutes}
                onChange={event =>
                  setForm(current =>
                    current
                      ? {
        ...
        <Card className="sbts-card">
          <CardHeader>
            ...
            <CardDescription>
              These checks remain gates, not extra user-facing phases.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {[vesselEntryReadinessGate, finalReinstatementGate].map(gate => (
              <div
                key={gate.key}
              ...
              >
                ...
                <div className="mt-1 text-xs text-slate-500">
                  {gate.requirements.length} mandatory checks configured in the
      ...
      <Card className="sbts-card">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base font-extrabold">
            <Gauge className="h-5 w-5 text-emerald-700" /> Gas-Test Acceptance
            Limits
          </CardTitle>
          <CardDescription>
            Configure plant-approved atmospheric acceptance limits. The runtime
            does not hard-code safety thresholds and will reject active/valid
            tests until this configuration is approved.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div
            className={`flex items-center justify-between rounded-xl border px-4 py-3 ${form.gasTestLimitsConfigured ? "border-emerald-200 bg-emerald-50/60" : "border-amber-200 bg-amber-50/70"}`}
          >
            <div>
              <div className="text-sm font-bold text-slate-900">
                Site limits reviewed and configured
              </div>
              <div className="text-xs text-slate-600">
                Enable only after Operations/Safety approve the values below.
              </div>
            </div>
            <Switch
              checked={form.gasTestLimitsConfigured}
              onCheckedChange={value =>
                setForm(current =>
                  current
                    ? { ...current, gasTestLimitsConfigured: value }
                    : current
                )
              }
            />
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
            Oxygen minimum, oxygen maximum and maximum LEL are mandatory when
            limits are enabled. H₂S and CO become mandatory readings only when a
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="sbts-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base font-extrabold">
              <ShieldCheck className="h-5 w-5 text-teal-600" /> Authorized Gas
              Tester
            </CardTitle>
            <CardDescription>
              Choose the role allowed to create valid atmospheric gas-test
              records.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Select
                value={form.authorizedGasTesterRoleKey}
                onValueChange={value =>
                  setForm(current =>
                    current
                      ? { ...current, authorizedGasTesterRoleKey: value }
                      : current
                  )
                }
              >
                <SelectTrigger className="sbts-input">
                  <SelectValue placeholder="Select gas tester role" />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="rounded-xl border border-teal-100 bg-teal-50/50 p-3 text-xs text-teal-900">
              The selected role must also carry the{" "}
              <strong>workflow.record.gasTest</strong> permission. Admin retains
        ...
        <Card className="sbts-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base font-extrabold">
              <Layers3 className="h-5 w-5 text-cyan-700" /> Isolation Package
              Rules
            </CardTitle>
            <CardDescription>
              Standardize package identification and entry-authorization
              validity.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Input
                value={form.isolationPackageIdPrefix}
                maxLength={16}
                onChange={event =>
                  setForm(current =>
                    current
                      ? {
                          ...current,
                          isolationPackageIdPrefix: event.target.value
                            .toUpperCase()
                            .replace(/[^A-Z0-9_-]/g, ""),
                        }
                      : current
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                min={15}
                max={2880}
                value={form.entryReadinessValidityMinutes}
                onChange={event =>
                  setForm(current =>
                    current
                      ? {
            ...
            <div className="sm:col-span-2 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
              New package IDs must begin with{" "}
              <strong>{form.isolationPackageIdPrefix || "VIP"}-</strong>.
              Authorized entry readiness expires automatically after the
              configured period unless a shorter expiry is recorded.
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="sbts-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base font-extrabold">
              <Upload className="h-5 w-5 text-indigo-600" /> Evidence &
              Attachment Policy
            </CardTitle>
            <CardDescription>
              Control field evidence limits centrally. The server validates
              these values before storage.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                min={1}
                max={50}
                value={form.evidenceMaxFileSizeMb}
                onChange={event =>
                  setForm(current =>
                    current
                      ? {
                          ...current,
                          evidenceMaxFileSizeMb: Number(event.target.value),
                        }
                      : current
                  )
                }
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                rows={4}
                value={form.evidenceAllowedMimeTypesJson}
                onChange={event =>
                  setForm(current =>
                    current
                      ? {
                          ...current,
                          evidenceAllowedMimeTypesJson: event.target.value,
                        }
                      : current
                  )
                }
        ...
        <Card className="sbts-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base font-extrabold">
              <Wrench className="h-5 w-5 text-amber-600" /> Field Record
              Defaults
            </CardTitle>
            <CardDescription>
              Standard units and record editor behavior shared by Blind Detail
              and Isolation Package views.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label>Default torque unit</Label>
              <Select
                value={form.defaultTorqueUnit}
                onValueChange={(value: "N·m" | "ft·lbf") =>
                  setForm(current =>
                    current ? { ...current, defaultTorqueUnit: value } : current
                  )
                }
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="ft·lbf">ft·lbf</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label>Default pump pressure unit</Label>
              <Select
                value={form.defaultPumpPressureUnit}
                onValueChange={(value: "psi" | "bar") =>
                  setForm(current =>
                    current
                      ? { ...current, defaultPumpPressureUnit: value }
                      : current
                  )
                }
              >
            ...
            <div className="space-y-1.5 sm:col-span-2">
              <Label>Field record editor</Label>
              <Select
                value={form.fieldRecordEditorMode}
                onValueChange={(value: "dialog" | "inline") =>
                  setForm(current =>
                    current
                      ? { ...current, fieldRecordEditorMode: value }
                      : current
                  )
                }
              >
                ...
                <SelectContent>
                  <SelectItem value="dialog">
                    Dialog · focused field entry
                  </SelectItem>
                  <SelectItem value="inline">
                    Inline · future compact workflow mode
                  </SelectItem>
                </SelectContent>
              </Select>
              <p className="text-xs text-slate-500">
                Dialog mode is the production-supported option in Sprint 3.
                Inline is stored for future activation.
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="sbts-card">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base font-extrabold">
            <Palette className="h-5 w-5 text-cyan-700" /> Workflow UI & Safety
            Presentation
          </CardTitle>
          <CardDescription>
            Keep workflow density and safety emphasis consistent with the active
            application theme.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-2">
          <div className="space-y-1.5">
            <Label>Workflow UI density</Label>
            <Select
              value={form.workflowUiDensity}
              onValueChange={(value: "comfortable" | "compact") =>
                setForm(current =>
                  current ? { ...current, workflowUiDensity: value } : current
                )
              }
            >
              <SelectTrigger className="sbts-input">
              ...
              <SelectContent>
                <SelectItem value="comfortable">
                  Comfortable · control room / desktop
                </SelectItem>
                <SelectItem value="compact">
                  Compact · tablet / high-volume work
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1.5">
            <Label>Safety banner emphasis</Label>
            <Select
              value={form.safetyBannerMode}
              onValueChange={(value: "prominent" | "standard" | "compact") =>
                setForm(current =>
                  current ? { ...current, safetyBannerMode: value } : current
                )
              }
            >
              <SelectTrigger className="sbts-input">
      ...
      <div className="flex justify-end">
        <Button
          onClick={handleSave}
          disabled={updateMutation.isPending}
          className="gap-2 rounded-2xl bg-slate-950 px-6 font-bold text-white"
        >
          <Save className="h-4 w-4" />
          {updateMutation.isPending
            ? "Saving..."
            : "Save Workflow & Safety Policies"}
        </Button>
      </div>
    </div>
  );
...
function FeatureControlsTab() {
  const {
    ...
    isLoading,
    refetch,
  } = trpc.featureToggles.get.useQuery();
  ...
  const features = [
    {
      key: "enableWorkflowTab",
      label: "Workflow Tab",
      desc: "Phase timeline, approvals, and phase transitions",
    },
    {
      key: "enableComplianceTab",
      label: "Compliance Tab",
      desc: "Safety checklists, torque records, inspections, photo evidence",
    },
    {
      key: "enableFieldActionsTab",
      label: "Field Actions Tab",
      desc: "PTW, LOTO, risk assessment, field notes",
    },
    {
      key: "enableQrMobileTab",
      label: "QR & Mobile Tab",
      desc: "QR code generation and mobile verification",
    },
    {
      key: "enableHistoryTab",
      label: "History Tab",
      desc: "Change log and audit trail",
    },
    {
      key: "enableProgressRing",
      label: "Progress Ring",
      desc: "Circular progress indicator in blind header",
    },
    {
      key: "enableQuickActions",
      label: "Quick Actions",
      desc: "Action cards in Overview tab",
    },
    {
      key: "enableBreadcrumb",
      label: "Breadcrumb Navigation",
      desc: "Show breadcrumb path above blind detail",
    },
    {
      key: "enableSafetyChecklists",
      label: "Safety Checklists",
      desc: "Phase safety checklist section in Compliance",
    },
    {
      key: "enableTorqueRecords",
      label: "Torque Records",
      desc: "Bolt-by-bolt torque verification",
    },
    {
      key: "enableInspectionRecords",
      label: "Inspection Records",
      desc: "NDE, MTR, Leak Test records",
    },
    {
      key: "enablePtw",
      label: "Permit to Work (PTW)",
      desc: "Work permit management in Field Actions",
    },
    {
      key: "enableLoto",
      label: "Lockout/Tagout (LOTO)",
      desc: "Energy isolation verification",
    },
    {
      key: "enableRiskAssessment",
      label: "Risk Assessment",
      desc: "Hazard identification and controls",
    },
    {
      key: "enableFieldNotes",
      label: "Field Notes",
      desc: "Free-text field observations",
    },
    {
      key: "enableQrGeneration",
      label: "QR Generation",
      desc: "Generate QR tokens for field access",
    },
    {
      key: "enableMobileVerification",
      label: "Mobile Verification",
      desc: "Mobile read-only verification page",
    },
    {
      key: "enablePhotoEvidence",
      label: "Photo Evidence",
      desc: "Attach photos and documents as compliance evidence",
    },
    {
      key: "enableOfflineAccess",
      label: "Offline Access",
      desc: "Allow offline data caching for field use",
    },
    {
      key: "enableSlipBlindSurveys",
      label: "Slip Blind Surveys",
      desc: "Periodic survey tracking for slip blinds",
    },
    {
      key: "enableCertificates",
      label: "Certificates",
      desc: "Generate and print blind certificates",
    },
    {
      key: "enableExpiryTracking",
      label: "Expiry Tracking",
      desc: "Track and alert on blind expiry dates",
    },
  ] as const;
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <ToggleLeft className="w-5 h-5 text-primary" /> Blind Detail Hub
            Features
          </CardTitle>
          <CardDescription>
            Enable or disable individual features in the Blind Detail Hub.
            Disabled features will be hidden from all users.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
