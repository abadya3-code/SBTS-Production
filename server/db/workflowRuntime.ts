...
const legacyPhaseKeyByLabel: Record<
  BlindPhase,
  keyof typeof legacyPhaseMigrationHints
> = {
...
export type RuntimePhaseStatus =
  | "pending"
  | "current"
  | "completed"
  | "blocked"
...
export type WorkflowRuntimeView = {
  ...
  records: {
    ...
    entryReadiness: Array<{
      id: number;
      packageId: string;
      status: string;
      allRequiredBlindsActive: boolean;
      lotoActive: boolean;
      pressureZero: boolean;
      drainedAndPurged: boolean;
      gasTestAcceptable: boolean;
...
function parseCanonicalPhase(value: string): CanonicalPhaseKey {
  if (canonicalPhaseKeys.includes(value as CanonicalPhaseKey))
    return value as CanonicalPhaseKey;
...
export type WorkflowActorAccess = {
  roleKeys: string[];
  permissionKeys: string[];
};

export async function getWorkflowActorAccess(
  openId: string,
  systemRole: "user" | "admin"
): Promise<WorkflowActorAccess> {
  if (systemRole === "admin") {
  ...
  return {
    ...
    permissionKeys: Array.from(
      new Set(permissions.map(row => String(row.permissionKey)))
    ),
  };
}
...
export async function assertAnyWorkflowPermission(
  ...
  if (!permissionKeys.some(key => hasPermission(access.permissionKeys, key))) {
    throw new Error(
      `One of the following permissions is required: ${permissionKeys.join(", ")}.`
    );
  }
}
...
async function ensureFinalApprovalSteps(
  blindTag: string,
  projectId: string,
  blindType: string
) {
  ...
  const desired = [
    ...
    {
      roleKey: "mechanicalVerifier",
      sequence: 3,
      conditional: 1,
      required: !metalRequired,
    },
    {
    ...
    {
      roleKey: "operationsForeman",
      sequence: 4,
      conditional: operationsRequired ? 0 : 1,
      required: operationsRequired,
    },
  ];
  const existing = await db
    .select()
    .from(workflowApprovalSteps)
    .where(
      and(
  ...
  for (const step of desired) {
    ...
    if (!row) {
      await db.insert(workflowApprovalSteps).values({
        blindTag,
        projectId,
        phaseKey: "finalApprovalReturnToService",
        approvalRoleKey: step.roleKey,
...
async function getRoleRecipients(roleKeys: string[]): Promise<string[]> {
  ...
  return Array.from(
    new Set(rows.map(row => String(row.openId)).filter(Boolean))
  );
}
...
async function notifyWorkflowRoles(
  ...
  const recipients = (await getRoleRecipients(roleKeys)).filter(
    openId => openId !== input.actor.openId
  );
...
export async function ensureBlindWorkflowRuntime(
  projectId: string,
  blindTag: string
): Promise<void> {
  const db = await requireDb();
  const blindRows = await db
    .select()
    .from(blinds)
    .where(and(eq(blinds.projectId, projectId), eq(blinds.tag, blindTag)))
    .limit(1);
  const blind = blindRows[0];
  if (!blind) throw new Error("Blind was not found in this project.");

  await db
    .insert(projectWorkflowAssignments)
    .values({
      projectId,
  ...
  const existing = await db
    ...
    .where(eq(blindWorkflowRuntime.blindTag, blindTag))
    .limit(1);
  if (!existing[0]) {
  ...
  const checklistRows = canonicalWorkflowPhases.flatMap(phase =>
    phase.checklist.map((label, index) => ({
      blindTag,
      projectId,
      phaseKey: phase.key,
  ...
  await db
    .insert(blindChecklistResponses)
    .values(checklistRows)
    .onDuplicateKeyUpdate({
      set: {
        itemLabel: sql`VALUES(itemLabel)`,
        required: sql`VALUES(required)`,
        updatedAt: now,
      },
    });
...
async function loadGateContext(projectId: string, blindTag: string) {
  ...
  const [
    ...
    db
      .select()
      .from(blindChecklistResponses)
      .where(eq(blindChecklistResponses.blindTag, blindTag)),
    db
      .select()
      .from(workflowEvidenceAttachments)
      .where(eq(workflowEvidenceAttachments.blindTag, blindTag)),
    db
      .select()
      .from(permitRecords)
      .where(eq(permitRecords.blindTag, blindTag))
      .orderBy(desc(permitRecords.createdAt)),
    db
      .select()
      .from(lotoRecords)
      .where(eq(lotoRecords.blindTag, blindTag))
      .orderBy(desc(lotoRecords.createdAt)),
    db
      .select()
      .from(gasTestRecords)
      .where(eq(gasTestRecords.blindTag, blindTag))
      .orderBy(desc(gasTestRecords.testedAt)),
    db.select().from(torqueRecords).where(eq(torqueRecords.blindTag, blindTag)),
    db
      .select()
      .from(leakTestRecords)
      .where(eq(leakTestRecords.blindTag, blindTag))
      .limit(1),
    db
      .select()
      .from(safetyHolds)
      .where(
        and(
          eq(safetyHolds.blindTag, blindTag),
          inArray(safetyHolds.status, ["active", "release_pending"])
        )
      )
      .orderBy(desc(safetyHolds.placedAt)),
    db
      .select()
      .from(workflowApprovalSteps)
      .where(eq(workflowApprovalSteps.blindTag, blindTag))
      .orderBy(workflowApprovalSteps.sequence),
    db
      .select()
      .from(isolationPackageBlinds)
      .where(eq(isolationPackageBlinds.blindTag, blindTag)),
  ]);
  if (!runtimeRows[0] || !blindRows[0])
    throw new Error("Workflow runtime could not be loaded.");
  ...
  if (membershipRows.length > 0) {
    ...
    packageRows = await db
      .select()
      .from(isolationPackages)
      .where(inArray(isolationPackages.id, packageIds));
    readinessRows = await db
      .select()
      .from(entryReadinessRecords)
      .where(inArray(entryReadinessRecords.packageId, packageIds))
      .orderBy(desc(entryReadinessRecords.createdAt));
  }
  return {
    ...
    gasRows,
    torqueRows,
    leak: leakRows[0] ?? null,
    activeHold: holdRows[0] ?? null,
    approvalRows,
    membershipRows,
    packageRows,
    readinessRows,
 ...
 */
export async function reconcileIsolationPackagesForBlind(
  blindTag: string
): Promise<void> {
  const db = await requireDb();
  ...
  for (const membership of memberships) {
    const linked = await db
      .select()
      .from(isolationPackageBlinds)
      .where(eq(isolationPackageBlinds.packageId, membership.packageId));
    const requiredTags = linked
      .filter(row => row.required === 1)
      .map(row => row.blindTag);
    if (requiredTags.length === 0) continue;
    const [runtimeRows, activeHolds, readinessRows] = await Promise.all([
      db
        .select()
        .from(blindWorkflowRuntime)
        .where(inArray(blindWorkflowRuntime.blindTag, requiredTags)),
      db
        .select({ blindTag: safetyHolds.blindTag })
        .from(safetyHolds)
        .where(
          and(
      ...
      db
        .select()
        .from(entryReadinessRecords)
        .where(eq(entryReadinessRecords.packageId, membership.packageId))
        .orderBy(desc(entryReadinessRecords.createdAt)),
    ]);
    ...
    const everyLifecycleIn = (allowed: string[]) =>
      requiredTags.every(tag => {
        const value = lifecycleByTag.get(tag);
        return Boolean(value && allowed.includes(value));
      });
    const anyLifecycleIn = (allowed: string[]) =>
      requiredTags.some(tag => {
        const value = lifecycleByTag.get(tag);
        return Boolean(value && allowed.includes(value));
      });
    const authorizedEntry = readinessRows.some(
      row =>
        row.status === "authorized" && (!row.validUntil || row.validUntil > now)
    );
    ...
    else if (everyLifecycleIn(["READY_FOR_SERVICE", "CLOSED"]))
...
function latestPermit(
  rows: Awaited<ReturnType<typeof loadGateContext>>["permitRows"],
  type: string
) {
...
function latestGas(
  rows: Awaited<ReturnType<typeof loadGateContext>>["gasRows"],
  purpose: string
) {
  return rows.find(
    row => row.testPurpose.toLowerCase() === purpose.toLowerCase()
  );
}
...
export async function evaluateWorkflowGate(
  ...
  if (context.runtime.isLocked === 1) {
    reasons.push({
      ...
      message: "This workflow record is locked and cannot be advanced.",
      source: "workflow",
    });
  }
  if (context.activeHold) {
    reasons.push({
      code: "SAFETY_HOLD_ACTIVE",
      message: `Safety Hold is active: ${context.activeHold.description}`,
      source: "hold",
    });
  }
  ...
  if (
    policy.enforceServerGates === 1 &&
    !allRequiredChecklistItemsComplete(
      phaseChecklist,
      getCanonicalPhase(phaseKey).checklist.length
    )
  ) {
    reasons.push({
      code: "CHECKLIST_INCOMPLETE",
      message:
        "All mandatory checklist items for the current phase must be completed.",
      source: "checklist",
    });
  }
  if (policy.requireEvidenceBeforePhaseSubmit === 1) {
    const phaseEvidence = context.evidenceRows.filter(
    ...
    if (phaseEvidence.length === 0) {
      reasons.push({
        ...
        message:
          "At least one controlled evidence attachment is required for the current phase.",
        source: "workflow",
      });
    }
  }
  ...
  const permitStatusUsable = (row: typeof ptw) =>
    Boolean(row && (row.status === "active" || row.status === "valid"));
  const activePtw =
    ptw &&
    permitStatusUsable(ptw) &&
    (policy.blockTransitionWhenPermitExpired !== 1 ||
      !ptw.validUntil ||
      ptw.validUntil > now);
  const activeLineBreaking =
    lineBreaking &&
    permitStatusUsable(lineBreaking) &&
    (policy.blockTransitionWhenPermitExpired !== 1 ||
      !lineBreaking.validUntil ||
      lineBreaking.validUntil > now);
  const activeLoto =
    loto && (loto.status === "active" || loto.status === "valid");

  const requireActivePtw = phaseKey !== "finalApprovalReturnToService";
  if (policy.requirePtwActive === 1 && requireActivePtw && !activePtw) {
    reasons.push({
      code: "PTW_NOT_ACTIVE",
      message: "A valid active PTW is required.",
      source: "permit",
    });
  }
  if (
    policy.requireLineBreakingPermit === 1 &&
    [
    ...
    reasons.push({
      code: "LINE_BREAKING_PERMIT_NOT_ACTIVE",
      message: "A valid Line Breaking Permit is required.",
      source: "permit",
    });
  }
  if (
    policy.requireLotoActive === 1 &&
    phaseKey !== "finalApprovalReturnToService"
  ) {
    if (!activeLoto)
      reasons.push({
        code: "LOTO_NOT_ACTIVE",
        message: "LOTO must remain active for this phase.",
        source: "loto",
      });
    if (!loto || loto.zeroEnergyVerified !== 1)
      reasons.push({
        code: "LOTO_ZERO_ENERGY_NOT_VERIFIED",
        message: "Zero-energy verification is required.",
        source: "loto",
      });
  }
  ...
  const checkGas = (purpose: string, required: boolean) => {
    ...
    if (!gas) {
      reasons.push({
        ...
        message: `A ${purpose} gas test is required.`,
        source: "gasTest",
      });
      return;
    }
    if (gas.status === "rejected" || gas.status === "cancelled") {
      reasons.push({
        code: "GAS_TEST_REJECTED",
        message: `The ${purpose} gas test is not acceptable.`,
        source: "gasTest",
      });
    } else if (
      !isRecordValidAt(gas.status, gas.validUntil, now) ||
      (policy.gasTestRequiresInstrumentCalibration === 1 &&
        (!gas.instrumentId ||
          !gas.calibrationExpiry ||
          isDateExpired(gas.calibrationExpiry, now)))
    ) {
      reasons.push({
        code: "GAS_TEST_EXPIRED",
        message: `The ${purpose} gas test or instrument calibration has expired.`,
        source: "gasTest",
      });
    } else {
      ...
      if (!acceptance.acceptable) {
        reasons.push({
          code: "GAS_TEST_OUT_OF_LIMITS",
          message: `${purpose} gas test is outside the configured plant limits: ${acceptance.reasons.join(" ")}`,
          source: "gasTest",
        });
      }
    }
  };
  if (["operationsInitialIsolation", "blindInstallation"].includes(phaseKey))
    checkGas("lineBreaking", policy.requireGasTestForLineBreaking === 1);
  if (phaseKey === "internalInspection")
    checkGas("entry", policy.requireGasTestForEntry === 1);
  if (
    ["reinstatementPreparation", "blindRemovalReinstatement"].includes(phaseKey)
  )
    checkGas("deblinding", policy.requireGasTestForDeBlinding === 1);
  ...
  const checkTorque = (
    ...
    if (!torque) {
      reasons.push({
        code: "TORQUE_RECORD_MISSING",
        message: `A ${stage} torque record is required.`,
        source: "torque",
      });
      return;
    }
    if (
      acceptedRequired
        ...
        : !["submitted", "accepted"].includes(torque.status)
    ) {
      reasons.push({
        code: "TORQUE_NOT_ACCEPTED",
        message: acceptedRequired
          ? `The ${stage} torque record must be independently accepted.`
          : `The ${stage} torque record must be submitted before progression.`,
        source: "torque",
      });
    }
    if (
      policy.requireTorqueCalibration === 1 &&
      (!torque.calibrationExpiry ||
        isDateExpired(torque.calibrationExpiry, now))
    ) {
      reasons.push({
        code: "CALIBRATION_EXPIRED",
        message: `Valid torque-tool calibration is required for ${stage}.`,
        source: "torque",
      });
    }
  };
  if (phaseKey === "blindInstallation")
    checkTorque("installation", policy.requireInstallationTorque === 1, false);
  if (phaseKey === "mechanicalVerification")
    checkTorque("installation", policy.requireInstallationTorque === 1, true);
  if (phaseKey === "blindRemovalReinstatement")
    checkTorque(
      "reinstatement",
      policy.requireReinstatementTorque === 1,
      false
    );
  if (phaseKey === "reinstatementVerification")
    checkTorque("reinstatement", policy.requireReinstatementTorque === 1, true);

  if (
    phaseKey === "mechanicalVerification" &&
    policy.requireIndependentVerifier === 1 &&
    actor
  ) {
    const db = await requireDb();
    const previous = await db
      ...
      .limit(1);
    if (
      previous[0]?.completedByOpenId &&
      previous[0].completedByOpenId === actor.openId
    ) {
      reasons.push({
        code: "INDEPENDENT_VERIFIER_REQUIRED",
        message:
          "The installation executor cannot perform the independent mechanical verification.",
        source: "workflow",
      });
    }
  }
  ...
  if (
    phaseKey === "internalInspection" &&
    policy.requireIsolationPackageForEntry === 1
  ) {
    if (context.membershipRows.length === 0) {
      reasons.push({
        code: "ISOLATION_PACKAGE_REQUIRED",
        message:
          "The blind must be linked to an Isolation Package before vessel entry readiness can be authorized.",
        source: "package",
      });
    } else {
      const authorized = context.readinessRows.some(
        row =>
      ...
      if (!authorized)
        reasons.push({
          code: "ENTRY_READINESS_NOT_AUTHORIZED",
          message: "Vessel Entry Readiness is not authorized or has expired.",
          source: "package",
        });
    }
  }
  ...
  if (phaseKey === "internalInspection") {
    const db = await requireDb();
    const mandatoryTemplates = await db
      .select({
        id: inspectionActivityTemplates.id,
        name: inspectionActivityTemplates.name,
      })
      .from(inspectionActivityTemplates)
      .where(
        and(
    ...
    if (mandatoryTemplates.length > 0) {
      const activityRows = await db
        .select()
        .from(inspectionActivityRecords)
        .where(
          and(
            eq(inspectionActivityRecords.blindTag, blindTag),
            inArray(
              inspectionActivityRecords.templateId,
              mandatoryTemplates.map(row => row.id)
            )
          )
        );
      ...
      const missing = mandatoryTemplates.filter(
        template =>
          !activityRows.some(
            row =>
      ...
      if (missing.length > 0) {
        reasons.push({
          code: "INSPECTION_ACTIVITIES_INCOMPLETE",
          message: `Mandatory inspection activities are incomplete: ${missing.map(row => row.name).join(", ")}.`,
          source: "inspection",
        });
      }
    }
  ...
  if (phaseKey === "internalInspection") {
    ...
    const [defects, punches, ndt] = await Promise.all([
      db
        .select()
        .from(defectNotifications)
        .where(
          and(
            eq(defectNotifications.projectId, projectId),
            eq(defectNotifications.blindTag, blindTag)
          )
        ),
      db
        .select()
        ...
        .where(
          and(
            ...
            eq(punchItems.blindTag, blindTag)
          )
        ),
      db
        .select()
        ...
        .where(
          and(
            eq(ndtRecords.projectId, projectId),
            eq(ndtRecords.blindTag, blindTag)
          )
        ),
    ]);
    if (policy.requireDefectDispositionBeforeClosure === 1) {
      const openDefects = defects.filter(
        row =>
          !["accepted_as_is", "closed", "transferred", "cancelled"].includes(
            row.status
          ) || !row.disposition?.trim()
      );
      if (openDefects.length)
        reasons.push({
          code: "DEFECT_DISPOSITION_INCOMPLETE",
          message: `${openDefects.length} defect notification(s) require controlled disposition.`,
          source: "inspection",
        });
    }
    if (policy.requireMandatoryPunchClosureBeforeReadyForClosure === 1) {
      const openPunches = punches.filter(
        row =>
          row.mandatory === 1 &&
          !["closed", "transferred", "cancelled"].includes(row.status)
      );
      if (openPunches.length)
        reasons.push({
          code: "MANDATORY_PUNCH_ITEMS_OPEN",
          message: `${openPunches.length} mandatory punch item(s) remain open.`,
          source: "inspection",
        });
    }
    if (policy.requireNdtAcceptanceBeforeReadyForClosure === 1) {
      const requiredNdt = defects.filter(row => row.requiresNdt === 1).length;
      const passedNdt = ndt.filter(row =>
        ["passed", "cancelled"].includes(row.status)
      ).length;
      const unresolvedNdt = ndt.filter(row =>
        ["planned", "in_progress", "failed", "retest_required"].includes(
          row.status
        )
      );
      if (unresolvedNdt.length || passedNdt < requiredNdt)
        reasons.push({
          code: "NDT_ACCEPTANCE_INCOMPLETE",
          message:
            "Required NDT records are missing, incomplete, failed, or awaiting retest.",
          source: "inspection",
        });
    }
  }
  ...
  if (phaseKey === "reinstatementVerification") {
    if (
      policy.certificateRequiresLeakTest === 1 &&
      (!context.leak ||
        context.leak.status !== "passed" ||
        context.leak.noLeakObserved !== 1)
    ) {
      reasons.push({
        code: "LEAK_TEST_NOT_PASSED",
        message:
          "A passed leak/service test with no leakage observed is required.",
        source: "leakTest",
      });
    }
    if (policy.requireIndependentVerifier === 1 && actor) {
      ...
      if (
        ...
        reasons.push({
          code: "INDEPENDENT_VERIFIER_REQUIRED",
          message:
            "The reinstatement executor cannot perform the independent final verification.",
          source: "workflow",
        });
      }
    }
  ...
  if (phaseKey === "finalApprovalReturnToService") {
    const pending = context.approvalRows.filter(
      row => row.status !== "approved" && row.status !== "not_required"
    );
    if (pending.length > 0)
      reasons.push({
        code: "FINAL_APPROVALS_INCOMPLETE",
        message: `Final approval chain is incomplete (${pending.map(row => row.approvalRoleKey).join(", ")}).`,
        source: "approval",
      });
    if (
      policy.requireLotoReleasedForCloseout === 1 &&
      (!loto || loto.status !== "closed" || !loto.releasedAt)
    ) {
      reasons.push({
        code: "LOTO_NOT_RELEASED",
        message:
          "LOTO closeout and controlled release must be recorded before final closure.",
        source: "loto",
      });
    }
  }
...
export async function getBlindWorkflowRuntimeView(
  ...
  const [phaseRows, checklistRows, evidenceRows] = await Promise.all([
    db
      .select()
      .from(blindPhaseInstances)
      .where(eq(blindPhaseInstances.blindTag, blindTag))
      .orderBy(blindPhaseInstances.sortOrder),
    db
      .select()
      .from(blindChecklistResponses)
      .where(
        and(
          eq(blindChecklistResponses.blindTag, blindTag),
          eq(blindChecklistResponses.phaseKey, phaseKey)
        )
      )
      .orderBy(blindChecklistResponses.id),
    db
      .select()
      .from(workflowEvidenceAttachments)
      .where(
        and(
          eq(workflowEvidenceAttachments.blindTag, blindTag),
          eq(workflowEvidenceAttachments.phaseKey, phaseKey)
        )
      )
      .orderBy(desc(workflowEvidenceAttachments.createdAt)),
  ]);
  ...
  return {
    runtime: {
      ...
      lifecycleStatus: context.runtime
        .lifecycleStatus as WorkflowLifecycleState,
    ...
    phases: phaseRows.map(row => ({
      ...
      shortLabel: getCanonicalPhase(parseCanonicalPhase(row.phaseKey))
        .shortLabel,
    ...
    records: {
      permits: context.permitRows.map(row => ({
        id: row.id,
        type: row.permitType,
        number: row.permitNumber,
        status: row.status,
      ...
      loto: context.lotoRows[0]
        ? {
            ...
            lockNumbers: (() => {
              try {
                return JSON.parse(
                  context.lotoRows[0].lockNumbersJson || "[]"
                ) as string[];
              } catch {
                return [];
              }
      ...
      gasTests: context.gasRows.map(row => ({
        ...
        oxygenPercent: row.oxygenPercent ?? null,
        lelPercent: row.lelPercent ?? null,
        h2sPpm: row.h2sPpm ?? null,
        coPpm: row.coPpm ?? null,
        notes: row.notes ?? null,
      })),
      torque: context.torqueRows.map(row => ({
        ...
        stage: row.stage,
        status: row.status,
        procedureReference: row.procedureReference ?? null,
        toolType: row.toolType,
        toolSerialNumber: row.toolSerialNumber ?? null,
        calibrationCertificateNumber: row.calibrationCertificateNumber ?? null,
        calibrationExpiry: row.calibrationExpiry ?? null,
        targetTorque: row.targetTorque ?? null,
        actualTorque: row.actualTorque ?? null,
        torqueUnit: row.torqueUnit,
        pumpPressure: row.pumpPressure ?? null,
        pumpPressureUnit: row.pumpPressureUnit ?? null,
        passes: (() => {
          try {
            return JSON.parse(row.passesJson || "[]") as unknown[];
          } catch {
            return [];
          }
        })(),
        witnessOpenId: row.witnessOpenId ?? null,
        notes: row.notes ?? null,
      })),
      leakTest: context.leak
        ? {
            id: context.leak.id,
            status: context.leak.status,
            testType: context.leak.testType ?? null,
            testMedium: context.leak.testMedium ?? null,
            testPressure: context.leak.testPressure ?? null,
            pressureUnit: context.leak.pressureUnit ?? null,
            durationMinutes: context.leak.durationMinutes ?? null,
            noLeakObserved: context.leak.noLeakObserved === 1,
            testedAt: context.leak.testedAt ?? null,
            acceptedAt: context.leak.acceptedAt ?? null,
            notes: context.leak.notes ?? null,
          }
        : null,
      isolationPackages: context.packageRows.map(row => ({
        id: row.id,
      ...
      entryReadiness: context.readinessRows.map(row => ({
        ...
        status: row.status,
        allRequiredBlindsActive: row.allRequiredBlindsActive === 1,
        lotoActive: row.lotoActive === 1,
        pressureZero: row.pressureZero === 1,
        drainedAndPurged: row.drainedAndPurged === 1,
        gasTestAcceptable: row.gasTestAcceptable === 1,
        confinedSpacePermitValid: row.confinedSpacePermitValid === 1,
        operationsApproved: row.operationsApproved === 1,
        entrySupervisorApproved: row.entrySupervisorApproved === 1,
        validUntil: row.validUntil ?? null,
        approvedAt: row.approvedAt ?? null,
      })),
    },
    ...
    activeHold: context.activeHold
      ? {
          ...
          previousLifecycleStatus:
            context.activeHold.previousLifecycleStatus ?? null,
          releaseRequestedByOpenId:
            context.activeHold.releaseRequestedByOpenId ?? null,
          releaseRequestedAt: context.activeHold.releaseRequestedAt ?? null,
          placedAt: context.activeHold.placedAt,
        }
      : null,
    permissions: {
      canExecuteCurrentAction: hasPermission(
        access.permissionKeys,
        phase.requiredPermissionKey
      ),
      canPlaceHold:
        policy.safetyHoldEnabled === 1 &&
        hasPermission(access.permissionKeys, "workflow.safety.hold"),
      canReleaseHold: hasPermission(
        access.permissionKeys,
        "workflow.safety.release"
      ),
      canOverride:
        actor.role === "admin" && policy.allowAdminWorkflowOverride === 1,
      canManagePermit: hasPermission(
        access.permissionKeys,
        "workflow.record.permit"
      ),
      canManageLoto: hasPermission(
        access.permissionKeys,
        "workflow.record.loto"
      ),
      canRecordGasTest: hasPermission(
        access.permissionKeys,
        "workflow.record.gasTest"
      ),
      canManageTorque:
        hasPermission(
          access.permissionKeys,
      ...
      canSubmitInstallationTorque: hasPermission(
        access.permissionKeys,
        "workflow.phase.installation.submit"
      ),
      canSubmitReinstatementTorque: hasPermission(
        access.permissionKeys,
        "workflow.phase.reinstatement.submit"
      ),
      canVerifyInstallationTorque: hasPermission(
        access.permissionKeys,
        "workflow.phase.mechanical.verify"
      ),
      canVerifyReinstatementTorque: hasPermission(
        access.permissionKeys,
        "workflow.phase.reinstatement.verify"
      ),
      canManageLeakTest: hasPermission(
        access.permissionKeys,
        "workflow.record.leakTest"
      ),
      canManagePackage: hasPermission(
        access.permissionKeys,
        "workflow.package.manage"
      ),
      canPrepareEntry: hasPermission(
        access.permissionKeys,
        "workflow.entry.prepare"
      ),
      canAuthorizeEntry: hasPermission(
        access.permissionKeys,
        "workflow.entry.authorize"
      ),
      canRecordApproval:
        hasPermission(access.permissionKeys, "workflow.phase.final.approve") ||
        hasPermission(
          access.permissionKeys,
          "workflow.phase.returnToService.authorize"
        ),
      canManageEvidence: hasPermission(
        access.permissionKeys,
        "workflow.record.evidence"
      ),
      canManageInspection: hasPermission(
        access.permissionKeys,
        "workflow.record.inspection"
      ),
      canApproveInspection: hasPermission(
        access.permissionKeys,
        "workflow.inspection.approve"
      ),
      canRecordDefect: hasPermission(
        access.permissionKeys,
        "workflow.quality.defect.record"
      ),
      canReviewDefect: hasPermission(
        access.permissionKeys,
        "workflow.quality.defect.review"
      ),
      canManagePunch: hasPermission(
        access.permissionKeys,
        "workflow.quality.punch.manage"
      ),
      canVerifyPunch: hasPermission(
        access.permissionKeys,
        "workflow.quality.punch.verify"
      ),
      canRecordNdt: hasPermission(
        access.permissionKeys,
        "workflow.quality.ndt.record"
      ),
      canReviewNdt: hasPermission(
        access.permissionKeys,
        "workflow.quality.ndt.review"
      ),
      canIssueCertificate: hasPermission(
        access.permissionKeys,
        "workflow.certificate.issue"
      ),
      canReissueCertificate: hasPermission(
        access.permissionKeys,
        "workflow.certificate.reissue"
      ),
      canRevokeCertificate: hasPermission(
        access.permissionKeys,
...
export async function updateWorkflowChecklistItem(
  input: {
    projectId: string;
    blindTag: string;
  ...
  const runtimeRows = await db
    ...
    .limit(1);
  if (!runtimeRows[0] || runtimeRows[0].currentPhaseKey !== input.phaseKey)
    throw new Error("Checklist item does not belong to the current phase.");
  const phase = getCanonicalPhase(input.phaseKey);
  const access = await getWorkflowActorAccess(actor.openId, actor.role);
  if (!hasPermission(access.permissionKeys, phase.requiredPermissionKey))
    throw new Error(`Permission ${phase.requiredPermissionKey} is required.`);
  if (input.phaseKey === "internalInspection") {
    ...
    if (policy.requireIsolationPackageForEntry === 1) {
      ...
      if (!authorized)
        throw new Error(
          "Vessel Entry Readiness must be authorized before inspection work can be recorded."
        );
    }
  }
  const checklistResult = await db.transaction(async tx => {
    const checklistItemRows = await tx
      .select({ id: blindChecklistResponses.id })
      .from(blindChecklistResponses)
      .where(
        and(
      ...
      .limit(1);
    if (!checklistItemRows[0]) {
      throw new Error(
        "Checklist item was not found for the current Blind phase."
      );
    }

    const now = new Date();
    await tx
      .update(blindChecklistResponses)
      .set({
      ...
      .where(eq(blindChecklistResponses.id, checklistItemRows[0].id));
    ...
    const checklistState = rows.map(row => ({
      required: row.required === 1,
      completed: row.completed === 1,
    }));
    const requiredCount = checklistState.filter(row => row.required).length;
    const completedCount = checklistState.filter(
      row => row.required && row.completed
    ).length;
    const complete = allRequiredChecklistItemsComplete(
      checklistState,
      phase.checklist.length
    );
    await tx
      .update(blindPhaseInstances)
      .set({ checklistComplete: complete ? 1 : 0, updatedAt: now })
      .where(
        and(
          eq(blindPhaseInstances.blindTag, input.blindTag),
          eq(blindPhaseInstances.phaseKey, input.phaseKey)
        )
      );
    return { complete, completedCount, requiredCount };
  });
  if (input.phaseKey === "internalInspection" && input.completed) {
    await db
      .update(blindWorkflowRuntime)
      .set({ lifecycleStatus: "WORK_IN_PROGRESS", updatedAt: new Date() })
      .where(eq(blindWorkflowRuntime.blindTag, input.blindTag));
    await reconcileIsolationPackagesForBlind(input.blindTag);
  }
  return {
    success: true,
    checklistComplete: checklistResult.complete,
    completedCount: checklistResult.completedCount,
    requiredCount: checklistResult.requiredCount,
  };
}

export async function transitionBlindWorkflow(
  input: {
    projectId: string;
    blindTag: string;
  ...
  const policy = await getWorkflowPolicySettings();
  const reasons: WorkflowBlockingReason[] = [];
  if (expectedPhase !== currentPhaseKey)
    reasons.push({
      code: "ACTION_PHASE_MISMATCH",
      message: `Action ${input.actionKey} is not valid for ${currentPhaseKey}.`,
      source: "workflow",
    });
  if (context.runtime.recordVersion !== input.expectedRecordVersion)
    reasons.push({
  ...
  const evaluated = await evaluateWorkflowGate(
    input.projectId,
    input.blindTag,
    actor
  );
  ...
  const canOverride =
    input.override === true &&
    actor.role === "admin" &&
    policy.allowAdminWorkflowOverride === 1 &&
  ...
  if (reasons.length > 0 && !canOverride) {
    await db.insert(workflowTransitionEvents).values({
      ...
      gateSnapshotJson: JSON.stringify({
  ...
  const newLifecycle = final
    ? "CLOSED"
    : lifecycleWhilePhaseCurrent[nextPhaseKey];
  const compatibilityPhase =
    legacyCompatibilityPhase[nextPhaseKey ?? currentPhaseKey];
  const now = new Date();
    ...
    await tx
      ...
      .set({
        ...
        gateSnapshotJson: JSON.stringify({
          ...
          lifecycleAfterCompletion: completedLifecycle,
        }),
      ...
      .where(
        and(
          eq(blindPhaseInstances.blindTag, input.blindTag),
          eq(blindPhaseInstances.phaseKey, currentPhaseKey)
        )
      );
    ...
    if (nextPhaseKey) {
      await tx
        ...
        .where(
          and(
            eq(blindPhaseInstances.blindTag, input.blindTag),
            eq(blindPhaseInstances.phaseKey, nextPhaseKey)
          )
        );
    }

    const runtimeUpdateResult = await tx
      .update(blindWorkflowRuntime)
      .set({
      ...
      .where(
        and(
          ...
          eq(blindWorkflowRuntime.recordVersion, beforeVersion)
        )
      );
    const updateMeta = Array.isArray(runtimeUpdateResult)
      ? runtimeUpdateResult[0]
      : runtimeUpdateResult;
    if (
      typeof (updateMeta as { affectedRows?: number })?.affectedRows ===
        "number" &&
      (updateMeta as { affectedRows: number }).affectedRows === 0
    ) {
      throw new Error(
        "STALE_RECORD_VERSION: Workflow was updated by another user."
      );
    }

    await tx
      .update(blinds)
      .set({ phase: compatibilityPhase, updatedAt: now })
      .where(eq(blinds.tag, input.blindTag));
  ...
  const notificationRoles = nextPhaseKey
    ? [getCanonicalPhase(nextPhaseKey).ownerRoleKey]
    : ["coordinator", "operationsForeman"];
  await notifyWorkflowRoles(notificationRoles, {
    type: "workflow_transition",
    title: final
      ? `Workflow completed · ${input.blindTag}`
      : `Action required · ${getCanonicalPhase(nextPhaseKey!).shortLabel}`,
    body: final
...
export async function placeWorkflowSafetyHold(
  input: {
    projectId: string;
    blindTag: string;
    reasonCode: string;
  ...
  const db = await requireDb();
  const policy = await getWorkflowPolicySettings();
  if (policy.safetyHoldEnabled !== 1)
    throw new Error("Safety Hold is disabled by system policy.");
  const access = await getWorkflowActorAccess(actor.openId, actor.role);
  if (!hasPermission(access.permissionKeys, "workflow.safety.hold"))
    throw new Error("Permission workflow.safety.hold is required.");
  const runtimeRows = await db
    .select()
    .from(blindWorkflowRuntime)
    .where(eq(blindWorkflowRuntime.blindTag, input.blindTag))
  ...
  if (runtimeRows[0].isLocked === 1)
    throw new Error(
      "A locked/closed workflow cannot be placed on Safety Hold. Start a new controlled work record instead."
    );
  ...
  const existing = await db
    ...
    .where(
      and(
        eq(safetyHolds.blindTag, input.blindTag),
        inArray(safetyHolds.status, ["active", "release_pending"])
      )
    )
    .limit(1);
  ...
  await reconcileIsolationPackagesForBlind(input.blindTag);
  await notifyWorkflowRoles(
    [
      "operations",
      "safety",
      "coordinator",
      getCanonicalPhase(phaseKey).ownerRoleKey,
    ],
    {
...
export async function releaseWorkflowSafetyHold(
  input: {
    projectId: string;
    blindTag: string;
    holdId: number;
  ...
  const policy = await getWorkflowPolicySettings();
  const access = await getWorkflowActorAccess(actor.openId, actor.role);
  if (!hasPermission(access.permissionKeys, "workflow.safety.release"))
    throw new Error("Permission workflow.safety.release is required.");
  const holds = await db
    .select()
    .from(safetyHolds)
    .where(
      and(
        eq(safetyHolds.id, input.holdId),
        eq(safetyHolds.blindTag, input.blindTag)
      )
    )
    .limit(1);
  const hold = holds[0];
  if (!hold || !["active", "release_pending"].includes(hold.status))
    throw new Error("Active Safety Hold was not found.");

  const now = new Date();
  const requiresIndependentApproval =
    policy.holdReleaseRequiresIndependentApproval === 1;
  ...
  if (requiresIndependentApproval && hold.status === "release_pending") {
    const requesterOpenId =
      hold.releaseRequestedByOpenId ?? hold.releasedByOpenId;
    if (
      hold.placedByOpenId === actor.openId ||
      requesterOpenId === actor.openId
    ) {
      throw new Error(
        "Independent Safety Hold release approval must be completed by a different authorized person."
      );
    }
  }
  ...
  const previousLifecycle =
    hold.previousLifecycleStatus &&
    workflowLifecycleStates.includes(
  ...
  await db.transaction(async tx => {
    await tx
      ...
      .set({
        ...
        releasedByOpenId:
          hold.releaseRequestedByOpenId ??
          hold.releasedByOpenId ??
          actor.openId,
...
export async function recordWorkflowApproval(
  ...
  if (
    !runtimeRows[0] ||
    runtimeRows[0].currentPhaseKey !== "finalApprovalReturnToService"
  ) {
    throw new Error(
      "Final approvals can only be recorded during the Final Approval & Return to Service phase."
    );
  }
  const access = await getWorkflowActorAccess(actor.openId, actor.role);
  if (actor.role !== "admin" && !access.roleKeys.includes(input.roleKey))
    throw new Error(
      `Actor must hold role ${input.roleKey} to complete this approval.`
    );
  const rows = await db
    .select()
    .from(workflowApprovalSteps)
    .where(
      and(
        eq(workflowApprovalSteps.blindTag, input.blindTag),
        eq(workflowApprovalSteps.approvalRoleKey, input.roleKey)
      )
    )
    .limit(1);
  const step = rows[0];
  ...
  if (policy.requireSequentialFinalApprovals === 1) {
    const prior = await db
      ...
      .where(
        and(
          eq(workflowApprovalSteps.blindTag, input.blindTag),
          sql`${workflowApprovalSteps.sequence} < ${step.sequence}`
        )
      );
    if (
      prior.some(
      ...
      throw new Error("Previous approval steps must be completed first.");
  }
  await db
    .update(workflowApprovalSteps)
    .set({
  ...
  if (input.approved) {
    const nextSteps = await db
      .select()
      .from(workflowApprovalSteps)
      .where(eq(workflowApprovalSteps.blindTag, input.blindTag))
      .orderBy(workflowApprovalSteps.sequence);
