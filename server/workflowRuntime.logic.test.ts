...
describe("Sprint 2 canonical workflow runtime", () => {
  it("keeps exactly eight ordered canonical phases", () => {
    ...
    expect(getNextCanonicalPhase("operationsInitialIsolation")).toBe("blindInstallation");
  ...
  it("maps every action command to one authoritative phase", () => {
    ...
    expect(getCanonicalPhase("finalApprovalReturnToService").ownerRoleKey).toBe("operationsForeman");
    expect(getCanonicalPhase("finalApprovalReturnToService").requiredPermissionKey).toBe("workflow.phase.returnToService.authorize");
  });

  it("uses safe lifecycle projections across installation, entry and reinstatement", () => {
    expect(lifecycleAfterPhaseCompletion.operationsInitialIsolation).toBe("READY_FOR_BLIND_INSTALLATION");
    expect(lifecycleAfterPhaseCompletion.mechanicalVerification).toBe("ACTIVE_ISOLATION");
    expect(lifecycleWhilePhaseCurrent.internalInspection).toBe("ACTIVE_ISOLATION");
    expect(lifecycleAfterPhaseCompletion.reinstatementVerification).toBe("READY_FOR_SERVICE");
    expect(lifecycleAfterPhaseCompletion.finalApprovalReturnToService).toBe("CLOSED");
  });
  ...
  it("does not allow an incomplete mandatory checklist to pass", () => {
    expect(allRequiredChecklistItemsComplete([
  ...
  it("generates stable checklist keys and validates expiry", () => {
    expect(normalizeChecklistItemKey("Pressure verified zero", 3)).toBe("04-pressure-verified-zero");
    const now = new Date("2026-07-24T12:00:00.000Z");
    expect(isRecordValidAt("valid", new Date("2026-07-24T12:30:00.000Z"), now)).toBe(true);
    expect(isRecordValidAt("valid", new Date("2026-07-24T11:59:59.000Z"), now)).toBe(false);
    expect(isRecordValidAt("draft", new Date("2026-07-24T12:30:00.000Z"), now)).toBe(false);
  });

  it("blocks valid gas tests until plant limits are configured and enforces them afterwards", () => {
    const unconfigured = evaluateGasTestAcceptance({ oxygenPercent: 20.9, lelPercent: 0, h2sPpm: 0, coPpm: 0 }, {
    ...
    expect(evaluateGasTestAcceptance({ oxygenPercent: 20.9, lelPercent: 0, h2sPpm: 0, coPpm: 0 }, policy).acceptable).toBe(true);
    expect(evaluateGasTestAcceptance({ oxygenPercent: 18.5, lelPercent: 0, h2sPpm: 0, coPpm: 0 }, policy).acceptable).toBe(false);
  });
});
